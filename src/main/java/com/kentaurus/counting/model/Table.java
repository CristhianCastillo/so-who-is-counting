            ...
            if (!existElement(element.getObservedNumber()))
                if (isElementValid(element.getObservedNumber()))
                    if (isLocationListValid(element.getNumberLocationList()))
                        ...
                        throw new Exception(
                                String.format(PropertiesCache.MESSAGE_ERROR_INVALID_LIST,
                                        element.getObservedNumber()));
                else
                    throw new Exception(String.format(
                            PropertiesCache.MESSAGE_ERROR_INVALID_NUMBER_VIEW,
                            element.getObservedNumber()));
            else
                throw new Exception(
                        String.format(PropertiesCache.MESSAGE_ERROR_INVALID_NUMBER_EXIST,
                                element.getObservedNumber()));
        } catch (Exception ex) {
